...
services: 
  ...
  kube-apiserver:
    ...
    volumes:
      - /opt/k8s/pki:/etc/kubernetes/pki/
    ports:
      - 6443:6443
  ...
  kube-controller-manager:
    ...
    command:
      ...
      - --client-ca-file=/etc/kubernetes/pki/ca.crt
      - --cluster-cidr=10.244.0.0/162001:db8:42:0::/56
...
```bash
etcdctl -w table --cacert=/opt/k8s/pki/etcd/ca.crt --cert=/opt/k8s/pki/etcd/server.crt \
  --key=/opt/k8s/pki/etcd/server.key --endpoints 127.0.0.1:2379 endpoint status
```