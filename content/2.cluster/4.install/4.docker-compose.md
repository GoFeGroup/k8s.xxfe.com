...
cat > /etc/kubernetes/docker-compose.yaml << EOF
version: "3.3"
...
services: 
  etcd-server:
    ...
    volumes:
      - /etc/kubernetes/pki/etcd:/etc/etcd/pki/
...
```bash
etcdctl -w table --cacert=/etc/kubernetes/pki/etcd/ca.crt --cert=/etc/kubernetes/pki/etcd/server.crt \
  --key=/etc/kubernetes/pki/etcd/server.key --endpoints 127.0.0.1:2379 endpoint status
```